...
public class DoctorController {
    ...
    public Response login(String json) throws ClassNotFoundException, SQLException {
		...
		sendingjson = new JSONObject();
		if (!loginSuccess[0].equals("")) {
			sendingjson.put("session_id", loginSuccess[0]);
			sendingjson.put("doctor_id", loginSuccess[1]);
	...
	@Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
	public Response viewDoctor(String json) throws ClassNotFoundException, SQLException, JSONException, ParseException {
		Connection conn = DBConnection.getConnection();
		JSONObject receivedJson;
		JSONObject sendingjson = new JSONObject();
		...
		if (msg.equals("LoggedIn")) {
			...
			dm = ds.getDoctor(dm);		
			if (!dm.getDoctor_id().equals("")) {
				sendingjson.put("sts", 1);
				sendingjson.put("doctor_license_no", dm.getDoctor_license_no());
				sendingjson.put("doctor_name", dm.getDoctor_name());
				sendingjson.put("doctor_phone", dm.getDoctor_phone());
				sendingjson.put("doctor_email", dm.getDoctor_email());
				sendingjson.put("doctor_address", dm.getDoctor_address());
				sendingjson.put("doctor_gender", dm.getDoctor_gender());
				sendingjson.put("doctor_specialization", dm.getDoctor_specialization());
			} else {
				sendingjson.put("sts", 0);
				sendingjson.put("error", "There is an error in updating doctor details");
			}
