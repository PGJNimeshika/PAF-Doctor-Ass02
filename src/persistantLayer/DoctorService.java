...
public class DoctorService {
	...
	public String[] doctorLogin(String username, String password) throws SQLException {
		...
		PreparedStatement selectStmnt = conn.prepareStatement("SELECT doctor_id FROM doctor_logins WHERE BINARY doctor_username=? && BINARY doctor_password=SHA1(?)");
        ...
        returnValues[0] = hashString;
        returnValues[1] = doctor_id;
		return returnValues;
	} 
	...
	public boolean viewDoctor(String doctor_id) {
		try {
			PreparedStatement updateStmnt = conn.prepareStatement("UPDATE doctor_logins SET docotor_session=? WHERE doctor_id=?");
			updateStmnt.setString(1, "");
			updateStmnt.setString(2, doctor_id);
